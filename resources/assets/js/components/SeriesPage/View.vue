<template>
    <div>
        ...
        <retrieval-modal @retrieved="retrieveSeries" :show.sync="showRetrieveModal"></retrieval-modal>
        <div class="md-elevation-3" v-show="series">
            <div class="col s12 no-padding" v-if="series">
                <div class="backdrop-image"
                     :style="{ 'backgroundImage': 'url(' + $root.getImagePath(series.backdrop_path, 'w1280') + ')'}">
                    <i class="material-icons pointer" id="back-button" @click="$router.go(-1)">arrow_back</i>
                    <div class="poster-image"
                         :style="{ 'backgroundImage': 'url(' + $root.getImagePath(series.poster_path, 'w185') + ')' }"></div>
            ...
            <div class="col s12"> <!-- toolbar -->
                <div class="movie-header">
                    ...
                    <div class="tool-bar" v-if="$root.isAdmin">
                        <div @click="$root.$router.push('/series/' + series.id + '/edit')" class="flex flex-justify-center flex-align-center"><md-icon>edit</md-icon>&nbsp;&nbsp;Edit</div>
                        <template v-if="series">
                            <template v-if="series.pending_rental.length === 0">
                            ...
                            <template v-else>
                                <div @click="showRetrieveModal = true" class="flex flex-justify-center flex-align-center">
                                    <md-icon>assignment_ind</md-icon>&nbsp;&nbsp;Borrowed by {{ series.pending_rental[0].user.name }}
                                </div>
                            </template>
                        </template>
                        <md-menu md-direction="bottom-start" id="just-seen-menu">
                            <div md-menu-trigger style="position: relative; top: 3px;">Just seen <template v-if="series && series.last_seen">(last: {{ lastSeen }})</template></div>
                            <md-menu-content>
                        ...
                        <div style="margin-left: auto; margin-right: 0;" @click="deleteSeries" class="flex flex-justify-center flex-align-center">
            ...
            <div class="col s12">
                <div class="movie-body">
                    ...
                    <div class="overview">
                        <template v-if="series && series.overview">
                            <span class="md-body-1">{{ series.overview }}</span>
                            <br><br>
                            <div class="flex flex-justify-end ">
                                <span class="md-caption">{{ series.comment ? '*' + series.comment : '' }}</span>
                            </div>
                        </template>
                        <div class="genres" v-if="series && series.genres">
                            <md-chip v-for="genre in series.genres" :key="genre.id">
                                {{ genre.name }}
                            </md-chip>
                        </div>
                    </div>
            ...
            <div class="col s12 no-padding" id="actors">   <!-- actors -->
                <template v-if="readyToShowActors && showActors && series && series.actors && !showUserModal && !showRetrieveModal">
                    <swiper :options="swiperOption">
                        <swiper-slide v-for="actor in series.actors" :key="actor.id">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            deleteSeries() {
                this.$store.dispatch('SERIES_ACTION_DELETE', this.series.id);
            },
            selectUser(user) {
                this.showUserModal = false;
                this.$store.dispatch('SERIES_ACTION_BORROW', {id: this.series.id, user}).then(res => {
                    Vue.set(this.series, 'pending_rental', [res.data]);
                });
            },
            retrieveSeries(params) {
                ...
                this.$store.dispatch('SERIES_ACTION_RETRIEVE', {id: this.series.id, payload: params}).then(() => {
                    Vue.set(this.series, 'pending_rental', []);
                });
            },
        ...
        computed: {
            lastSeen() {
                if (!this.series.last_seen) return null;
                return moment(this.series.last_seen).fromNow();
            },
            tableContents() {
                if (!this.series) {
                    return [];
                ...
                let table = [
                    this.series.episode_runtime + ' min. <i class="material-icons table-icons">timer</i>',
                    this.series.first_air_date + ' <i class="material-icons table-icons">calendar_today</i>',
                ];
                if (this.series.based_on_book) {
                    table.push('Based on book <i class="material-icons table-icons"> check </i>')
                }
                if (this.series.true_story) {
                    table.push('True story <i class="material-icons table-icons"> check </i>')
                }
                if (this.series.blue_ray) {
